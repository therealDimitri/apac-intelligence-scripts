/**
 * Restore the canonical event_compliance_summary view (without exclusions)
 * This fixes the issue where clients were only showing 2 event types
 */
...
// Use session mode pooler (port 5432) for DDL operations
...
async function main() {
  ...
  try {
    ...
    // Step 3: Execute canonical view migration
    console.log('\n3. Executing canonical view migration...')

    // Drop existing view (will also drop client_health_summary due to CASCADE)
    ...
    console.log('   Creating canonical view (without exclusions)...')
    await sql.unsafe(`
    ...
    // Read and execute the health summary view
    await sql.unsafe(`
