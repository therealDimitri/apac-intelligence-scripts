/**
 * Restore the canonical event_compliance_summary view
 * Uses the database password directly with different connection options
 */
...
const connectionStrings = [
  // Transaction pooler with specific options
  ...
  // Direct database (newer format)
  `postgresql://postgres.${projectRef}:${DB_PASSWORD}@aws-0-ap-southeast-1.pooler.supabase.com:5432/postgres?pgbouncer=true`,
]
...
async function main() {
  console.log('\n=== Restoring Canonical Event Compliance View ===\n')

  // Step 1: Check current state
  console.log('1. Current state for Te Whatu Ora Waikato...')
  ...
  // Step 2: Try different connection strings
  console.log('\n2. Trying database connections...')
  ...
  if (!sql) {
    ...
    console.log('\nSQL file: docs/migrations/20251203_CANONICAL_event_compliance_view.sql')
    process.exit(1)
  }
  ...
  try {
    ...
    // Create the canonical view WITHOUT exclusion logic
    console.log('   Creating canonical view...')
    await sql.unsafe(`
    ...
    console.log('\n   ✅ Event compliance view restored successfully!')
  ...
  if (afterError) {
    ...
    console.log('   Event Type Names:', after.event_compliance?.map(e => e.event_type_name).join(', '))
