/**
 * Execute the materialized view migration using pooler connection
 */
...
// Use the pooler URL (Transaction mode may work for DDL)
...
const fullMigrationSQL = fs.readFileSync(migrationPath, 'utf-8')

async function main() {
  console.log('\n=== Running View Migration ===\n')
  ...
  if (before) {
    ...
    const healthCheck = before.event_compliance?.find(e => e.event_type_name === 'Health Check (Opal)')
    console.log('   Health Check (Opal):', healthCheck ? `${healthCheck.actual_count}/${healthCheck.expected_count}` : 'Not found')
  }

  // Step 2: Connect via pooler
  console.log('\n2. Connecting to database via pooler...')
  ...
  try {
    ...
    // Step 3: Execute migration in parts
    ...
    await sql`CREATE INDEX idx_event_compliance_client_year ON event_compliance_summary(client_name, year)`
    await sql`CREATE INDEX idx_event_compliance_cse ON event_compliance_summary(cse)`
    await sql`CREATE INDEX idx_event_compliance_year ON event_compliance_summary(year DESC)`
    await sql`CREATE INDEX idx_event_compliance_status ON event_compliance_summary(overall_status)`
    await sql`CREATE INDEX idx_event_compliance_segment ON event_compliance_summary(segment)`
    await sql`CREATE INDEX idx_event_compliance_cse_year ON event_compliance_summary(cse, year DESC)`
    await sql`CREATE UNIQUE INDEX idx_event_compliance_unique_client_year ON event_compliance_summary(client_name, year)`
  ...
  await new Promise(r => setTimeout(r, 3000)) // Wait for PostgREST reload
  ...
  if (afterError) {
    console.log('   Error fetching updated data:', afterError.message)
  } else if (after) {
