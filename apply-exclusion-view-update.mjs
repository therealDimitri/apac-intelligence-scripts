#!/usr/bin/env node
/**
 * Apply the exclusion view update via Supabase Pooler
 */
...
// Use pooler connection from DATABASE_URL
// For DDL operations (CREATE, DROP), we need session mode (port 5432) not transaction mode (port 6543)
...
// Convert transaction pooler (6543) to session pooler (5432) for DDL statements
if (connectionString.includes(':6543/')) {
  connectionString = connectionString.replace(':6543/', ':5432/')
  console.log('Converted to session pooler (port 5432) for DDL support')
} else {
...
const pool = new Pool({
  ...
  // Increase timeout for DDL operations
...
async function main() {
  console.log('\n=== Applying Exclusion View Update ===\n')

  try {
    const client = await pool.connect()
    console.log('✓ Connected to database')

    console.log('\n1. Recreating materialized view with exclusion support...')
    await client.query(viewSQL)
    console.log('   ✓ View recreated successfully')

    console.log('\n2. Verifying Department of Health - Victoria...')
    const result = await client.query(`
      SELECT client_name, overall_compliance_score, event_compliance, total_event_types_count
    ...
    if (result.rows.length > 0) {
      ...
      console.log('   Total Event Types:', row.total_event_types_count)
