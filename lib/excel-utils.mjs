 ...
 */
/**
 * Read a full row of monthly values from a sheet (columns C–N = Jan–Dec).
 * Returns an array of 12 values (null for missing cells).
 *
 * @param {object} sheet - XLSX worksheet object
 * @param {number} row - Row number (1-indexed)
 * @param {object} [opts]
 * @param {string[]} [opts.cols] - Column letters (default C–N)
 * @param {*} [opts.fallback=null] - Value for missing cells
 * @returns {Array<number|null>} 12-element array of monthly values
 */
export function readMonthlyRow(sheet, row, opts = {}) {
  const cols = opts.cols || ['C', 'D', 'E', 'F', 'G', 'H', 'I', 'J', 'K', 'L', 'M', 'N']
  const fallback = opts.fallback ?? null
  return cols.map((col) => getCellValue(sheet, `${col}${row}`, fallback))
}
